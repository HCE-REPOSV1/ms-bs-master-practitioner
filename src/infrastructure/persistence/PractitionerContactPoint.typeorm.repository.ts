...
export class PractitionerContactPointTypeOrmRepository implements PractitionerContactPointRepository {
  ...
  async update(id: number, data: Partial<PractitionerContactPoint>): Promise<PractitionerContactPoint | null> {
    await this.repo.update({ contact_point_id: id, is_active: true } as any, data as any);
